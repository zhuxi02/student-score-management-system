...
void count() {
	...
	student* p = head;
	while (p != NULL) {
		if (p->course >= 0 && p->paper >= 0) {
			p->total = p->course + p->paper;
		}
		else {
			p->total = -1;
		}
	...
	while (p != NULL) {
		...
		for (int i = 0; i < class_count; i++) {
			if (class_list[i] == p->cls) {
				exists = 1;
				break;
			}
		}
		if (!exists) {
			class_list[class_count++] = p->cls;
		}
		p = p->next;
	}
	for (int i = 0; i < class_count; i++) {
		int now_class = class_list[i];
		student* class_stu[500];
		int stu_count = 0;
		p = head;
		while (p != NULL) {
			if (p->cls == now_class && p->total != -1) {
				class_stu[stu_count++] = p;
			}
			p = p->next;
		}
		for (int a = 0; a < stu_count - 1; a++) {
			for (int b = 0; b < stu_count - a - 1; b++) {
				if (class_stu[b]->total < class_stu[b + 1]->total) {
					student* temp = class_stu[b];
					class_stu[b] = class_stu[b + 1];
					class_stu[b + 1] = temp;
				}
			}
		}
	...
	printf("总成绩与班级排名计算完成！\n");
	printf("规则：\n");
	printf("1. 总成绩 = 课程综合成绩 + 论文成绩\n");
	printf("2. 无有效成绩的学生总分与排名置为 -1\n");
	printf("3. 排名按【班级内总分从高到低】生成\n\n");
	system("pause");
}
void sortall() {
	system("cls");
	...
	for (int i = 0; i < total - 1; i++) {
		for (int j = 0; j < total - i - 1; j++) {
			int score1 = arr[j]->total;
			int score2 = arr[j + 1]->total;
			if (score1 < score2) {
				student* temp = arr[j];
				arr[j] = arr[j + 1];
				arr[j + 1] = temp;
...
void sortclass() {
	...
	printf("========== 班级 %d 学生成绩排序（从高到低）==========\n", c);
	printf("学号\t姓名\t班级\t课程\t论文\t总分\t班级排名\n");
	for (int i = 0; i < n; i++) {
		student* s = arr[i];
		printf("%d\t%s\t%d\t%d\t%d\t%d\t%d\n",
			s->id, s->name, s->cls, s->course, s->paper, s->total, s->rank);
	}
	system("pause");
}
void showall() {
